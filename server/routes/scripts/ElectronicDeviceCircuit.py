...
class EDC:
    ...
    def UJT(self):
        argument = self.arg[:]
        m1 = (float(argument[33])-float(argument[34]))/5
        m2 = (float(argument[35])-float(argument[36]))/10
        m = (m1+m2)/2
        print(json.dumps({"answer":[{"Thus the characteristics of uniJunction resistor was determined and intrinsic state are determined. Intrinsic state of radian = ":str(m) }]}))
    def Photo(self):
        argument = self.arg[:]
